...
class User(Base):
    ...
    plex_library_items: Mapped[list["PlexLibraryItem"]] = relationship(
        back_populates="user",
        cascade="all, delete-orphan",
    )
...
class UserPreferences(Base):
    ...
    plex_token: Mapped[str | None] = mapped_column(Text, nullable=True)
    plex_server_name: Mapped[str | None] = mapped_column(String(200), nullable=True)
    plex_server_uri: Mapped[str | None] = mapped_column(String(500), nullable=True)
    plex_server_token: Mapped[str | None] = mapped_column(String(200), nullable=True)
    plex_machine_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    plex_library_section_ids: Mapped[str | None] = mapped_column(Text, nullable=True)
    plex_sync_status: Mapped[str | None] = mapped_column(String(40), nullable=True)
    plex_sync_message: Mapped[str | None] = mapped_column(Text, nullable=True)
    plex_last_sync_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    plex_item_count: Mapped[int | None] = mapped_column(Integer, nullable=True)
    plex_webhook_secret: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class PlexLibraryItem(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "tmdb_id", "media_type", name="uq_plex_library_user_tmdb_media"),
    )
    ...
    plex_title: Mapped[str] = mapped_column(String, nullable=False)
    ...
    user: Mapped["User"] = relationship(back_populates="plex_library_items")
