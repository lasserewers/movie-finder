...
import type { WatchedItem } from "../api/watched";
...
export default function WatchedOverlay({ open, onClose, items, loading = false, onSelectMovie }: Props) {
  const [sortMode, setSortMode] = useState<SortMode>("watched_desc");
  ...
  const sortedItems = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      const direction = sortMode === "watched_asc" ? 1 : -1;
      const aWatched = toTimestamp(a.watched_at);
      const bWatched = toTimestamp(b.watched_at);
      if (aWatched === null && bWatched === null) return 0;
          ...
          >
            ...
            <div className="p-6 sm:p-8 pb-0 sm:pb-0">
              <div className="min-w-0 pr-12">
                <h3 className="font-display text-2xl">Watched</h3>
              ...
              <div className="flex flex-col gap-2 mt-3 w-[min(100%,360px)] max-sm:mx-auto">
                <div className="flex items-center justify-center sm:justify-start gap-2 w-full">
                  <label htmlFor="watched-sort" className="text-xs text-muted uppercase tracking-wide whitespace-nowrap">
                    Sort by
                  </label>
                  <select
                    id="watched-sort"
            ...
            <div className="flex-1 overflow-auto p-6 sm:p-8 pt-4 sm:pt-4">
              {loading ? (
                <div className="text-sm text-muted py-8">Loading watched titles...</div>
              ) : sortedItems.length === 0 ? (
                <div className="text-sm text-muted py-8">
                  {items.length === 0 ? "You have not marked any titles as watched yet." : "No titles match this filter."}
                </div>
              ) : (
